...
.get(cors.cors, authenticate.verifyUser, (req, res, next) => {
    ...
    .then((favorites) => {
        if (favorites) {
            ...
            if(!user_favorites) {
                var err = new Error('You have no favorite dish!');
                err.status = 404;
                return next(err);
            }
            res.statusCode = 200;
...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then((favorites) => {
        var user;
        if(favorites) {
            user = favorites.filter(fav => fav.user._id.toString() === req.user.toString())[0];
        }
        if(!user) {
        ...
        for (let i of req.body) {
            if (user.dishes.find((d_id) => {
                if(d_id._id) {
                    return d_id.id.toString() === i._id.toString();
                }
            }))
            continue;
...
.put(cors.corsWithOptions, authenticate.verifyUser, (rew, res, next) => {
    res.statusCode = 403;
    res.end('PUT operaton is not supported on /favorites');
})
    ...
    .then((favorites) => {
        if (favorites) {
            ...
            if(dish) {
                ...
                var err = new Error('You do not have a favorite dish ' + req.params.dishId);
                err.status = 404;
                return next(err);
            }
        }
...
.put(cors.corsWithOptions, authenticate.verifyUser, (rew, res, next) => {
    res.statusCode = 403;
    res.end('PUT operaton is not supported on /favorites/:dishId');
})
